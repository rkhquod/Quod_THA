...
class NeuralNetModel(MLModel):
    def __init__(self, input_dim, **kwargs):
        ...
        default_params = self.config.get('NeuralNet', {})
        ...
        for epoch in range(self.params["epochs"]):
            ...
            if val_dataloader is not None:
                ...
                if (epoch + 1) % self.params["print_per_epoch"] == 0 or (epoch + 1) == self.params["epochs"]:
                    print(f"Epoch [{epoch+1}/{self.params['epochs']}], Train Loss: {running_loss/len(dataloader):.4f}, Valid Loss: {valid_loss:.4f}")
                ...
                print(f"Epoch [{epoch+1}/{self.params['epochs']}], Train Loss: {running_loss/len(dataloader):.4f}")
